...
def main():
    ...
    video_path = os.path.join(save_path, f'tpv_and_pred.mp4')
    ...
    for file_name in file_names:
        ...
        COL_W = pcd_width
        # gap = np.ones((COL_W // 10, COL_W, 3), dtype=np.uint8) * 255
        gap = None
        GAP_W = COL_W // 20
        ...
        for modal in modals:
            ...
            occ_pred_path = os.path.join(vis_path, modal, 'sequences', sequence, 'occ_pred', 'top_down')
            ...
            assert occ_pred is not None and tpv_xy is not None and tpv_yz is not None and tpv_zx is not None
            assert tpv_yz.shape[:2] == tpv_zx.shape[:2]
            assert tpv_xy.shape[1] == tpv_yz.shape[1]
            ...
            canvas = np.concatenate((canvas, gap), axis=0)
            canvas = np.concatenate((canvas, occ_pred), axis=0)
        ...
        canvas_input_gt = np.concatenate((canvas_input_gt, gap), axis=0)
        canvas_input_gt = np.concatenate((canvas_input_gt, gt), axis=0)
        ...
        texts_positions_4 = [
            (
                "Visualizations for TPV Features and SSC Predictions (" + dataset.replace('itti', 'ITTI') + ')',
                (int(GAP_W * 17), int(0)),
            ),
        ]
